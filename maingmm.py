    ...
    def __init__(self, dims=(2, ), datatype=None, cfg=None, **dist_args):
        ...
        if self._bdist_type == 'mvn':
            ...
            if self._var_base_dist == True:
                ...
                self.dw = torch.nn.Parameter(torch.ones((self.nc,), dtype=torch.float32,device = self.device), requires_grad = True)
                print('TRUETRUE')
            else:
                ...
                self.dw = torch.nn.Parameter(torch.ones((self.nc,), dtype=torch.float32,device = self.device), requires_grad = False)
                print('FALSEFALSE')
