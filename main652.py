...
class Model(object):
    def __init__(self, dims=(2, ), datatype=None, cfg=None, **dist_args):
        ...
        if self._bdist_type == 'mvn':
            ...
            if self._var_base_dist == True:
                ...
                self.p = torch.nn.Parameter(torch.FloatTensor([0.4,1.2]).to(self.device) + dist_args['p'], requires_grad = False)
                ...
                self.p = torch.nn.Parameter(torch.FloatTensor([0.4,1.2], dtype=torch.float32,device = self.device) + dist_args['p'], requires_grad = False)
                self.dw = torch.nn.Parameter(torch.zeros((self.dimension,), dtype=torch.float32,device = self.device) + dist_args['dw'], requires_grad = False)
                print('FALSEFALSE')
    ...
    def train_on_batch(self, y, beta=-1, tbase=None):
        ...
        if self._bdist_type != 'mvggd':
            ...
            lossm = -1.0 * torch.sum(self.base_distribution.log_prob(z) + log_det_jacobian)
        else:
            #lossm = -1.0 * torch.mean(self.base_distribution.log_prob_agg(z) + log_det_jacobian)#MEANMEAN
            lossm = -1.0 * torch.sum(self.base_distribution.log_prob_agg(z) + log_det_jacobian)
        beta = beta
...
def main(cfg):
    ...
    if vvariable == True:
        ...
        for data in dataset:
            ...
            prefix = 'ddim_' + str(ddim) + '_dbeta_' + str(vdbeta) + '_prior_' + vprior + '_vnoise_' + str(vvariable) + '_nbeta_' + str(vnbeta) + '_nloss_' + nloss_type+ '_bloss_' + bloss_type + '_'
...
#---------------------------------------000000000000000000
        ...
        if ddistrib == 'credit':
            ...
            x = x.contiguous()            
            px = np.mean(np.exp(dataset.bdist.logpdf(x.detach().cpu().numpy())),axis=1)
            qx = model.log_py(x)
            #print('PX',px)
            #print('QX',qx)
        ...
        z, log_det_jacobian = model.net(x)
        z = z.view(y.size(0), -1)
        #print('!!!',self.base_distribution.log_prob(z).shape,log_det_jacobian.shape)
        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        if vprior != 'mvggd':
            nkl = -1.0 * torch.sum(model.base_distribution.log_prob(z) + log_det_jacobian)#MEANMEAN
        else:
            nkl = -1.0 * torch.sum(model.base_distribution.log_prob_agg(z) + log_det_jacobian)#MEANMEAN
...
#---------------------------------------000000000000000000
        ...
        step = start_step
        for data in dataset:
            prefix = 'ddim_' + str(ddim) + '_dbeta_' + str(vdbeta) + '_prior_' + vprior + '_vnoise_' + str(vvariable) + '_nbeta_' + str(vnbeta) + '_nloss_' + nloss_type + '_bloss_NonVar_'
            # training
            model.train()
            start_time = time.perf_counter()
            y = data
